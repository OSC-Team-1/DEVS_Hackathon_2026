...
function coordsToRowCol(lat: number, lng: number, gridInfo: GridInfo) {
...
function determineMinigameCount(colSpan: number): number {
    if (colSpan <= 10) return 1;
    if (colSpan <= 20) return 2;
    if (colSpan <= 50) return 3;
    return 4;
}
...
export function planJourney(
    ...
    const startTile = coordsToRowCol(
        startCity.lat,
        startCity.lng,
        worldGrid.info,
    );
    const endTile = coordsToRowCol(endCity.lat, endCity.lng, worldGrid.info);
